...
const addDevlopPolyfill = (html) => {
  return  html.replace(/<\/head>/g, `<script>
      ...
      onBeforeSetupMiddleware: function (devServer) {
        ...
        app.get('/', async function (req, res ,next) {
          ...
          for (const item of routerList) {
            ...
            let editUrl = item.edit ? `http://${ip}:${port}${item.path.replace(/\/$/, '')}/visualediting`:'';
            ...
            let split_a = `<a target="_blank" onclick='openSplit(\`${JSON.stringify(item)}\`)' style="color: #008CBA">Split${Object.keys(item.splitConfig).length === 0 ? '（未创建）' : ''}</a>`

            if(item.trunkName.indexOf('pc_')==0) {
              pcList.push(`<p>./src/${item.trunkName.replace(/_/g,'/')}: ${a} ${edit_a} ${ssr_a} ${__ui__a} ${split_a}</p>`);
            }
            if(item.trunkName.indexOf('mobile_')==0){
              mobileList.push(`<p>./src/${item.trunkName.replace(/_/g,'/')}:  ${a} ${edit_a} ${ssr_a} ${__ui__a} ${split_a}</p>`);
            }
          }
        ...
        app.post('/splitUpdate', async function (req, res, next) {
          const { splitData: { data: { dataMap: splitData, chipData, extraSetting }  }, customData, isSplitEmpty } = req.body
          // 更新节点信息到config_split.js中，没有会直接创建
          await writeFile(path.resolve(customData.appjsPath, '../config_split.js'), `module.exports = ${JSON.stringify(splitData)}`)
          ...
          const freezeChip = configJsContent.allData.transferV3 ? configJsContent.allData.transferV3.filter(c => !AvailableChipType.includes(c[2])) : []
          let newChip = []
          chipData.forEach(chip => {
            newChip.push([`${chip.chipData.name}:${chip.chipData.title}`, 'KVProxy', ChipTypeMap[chip.chipType], chip.chipId, {}])
          })
          configJsContent.allData.transferV3 = [...freezeChip, ...newChip]
          await writeFile(path.resolve(customData.appjsPath, '../config.js'), `module.exports = ${JSON.stringify(configJsContent, null, 2)}`)
        ...
        app.get('*', async function (req, res ,next) {
          ...
          if(/\/.*?.html/.test(req.path)) {
            let devAppPath = path.join(process.cwd(),'./src',req.path.replace(/_/g, '/').replace('.html','/.app.jsx')) ;
            if(req.path.includes('_edit')) {
              devAppPath = path.join(process.cwd(),'./src',req.path.replace(/_/g, '/').replace('.html','/.edit.jsx').replace('/edit/','/')) ;
            }
                ...
                if( ssr && router.ssr ) {
                  ...
                  if(ssrFile.length<=10) {
                    ...
                    ssrFile = appFile
                      ...
                      .replace(/\/\*[\w\W]*?\*\//g, "")
                      .replace(/ReactDOM.render.*/, `const server = function server(allData) {
...
export default server;`);
                ...
                let html = await request.get({url: `http://127.0.0.1:${port}/${router.trunkName}${req.edit ? '_edit' : ''}.html`,  timeout: 30 * 1000 });
                ...
                if(router.handler == true){
                  ...
                  let {response,body} = await request.post({url:`http://test0.ucms.ifeng.com/custompreview/${_path}`, qs, json: {code, params}, response: true});
                  if(response.statusCode == 400){
                    return  res.send(`<h4 style="color: red">handler.js发现错误</h4><p style="color: red">${body}</p>`)
                  }
                  if(body && body.next ===  false) {
                    return  Request.post( {url:`http://test0.ucms.ifeng.com/custompreview/${_path}`, qs, json: {code, params}}).pipe(res);
                   }
                  ...
                  let data = await request.post({url:'http://test0.web-feedflow-api.shank.ifeng.com/api/custom/dev/data', qs, json: allDataConfig});
                ...
                let ssrhtml = '';
                if(ssr){
                  html = html.replace(/(<div[\w\W]*?id="root"[\w\W]*?\>)[\w\W]*?(<\/div>)/, '$1<%- body %>$2');
                  try {
                    let ssrjs = '';
                    let ssrjsmap = '';
                    let ssrjsFilePath = ''
                    for(let i=0; i<10; i++){
                      ssrjs = await request.get({url: `http://127.0.0.1:${port}/${router.trunkName}_ssr.js`});
                      ssrjsmap = await request.get({url: `http://127.0.0.1:${port}/${router.trunkName}_ssr.js.map`});
                      if(ssrjs.includes('createElement') || i>=9 ) {
                        let ssrjsHash = md5(ssrjs);
                        ssrjsFilePath =  path.join(process.cwd(),`./dist/${ssrjsHash}.js`)
                        ssrjsmapFilePath =  path.join(process.cwd(),`./dist/${router.trunkName}_ssr.js.map`)
                ...
                if(router.tongji){
                  let tongji = await request.get({url:'http://test0.web-feedflow-api.shank.ifeng.com/api/custom/dev/tongji', json: true})
